...
public class IKLeg : MonoBehaviour {
    ...
    public bool superExtended;
    [SerializeField] float superExtendedRange = 3;
    ...
    private void LateUpdate() {
        if (!enabeled) return;
        smoothtarget = Vector3.Lerp(smoothtarget, Target.position, targetSmoothing);
    ...
    void IK() {
        ...
        if ((smoothtarget - bones[0].position).sqrMagnitude >= Length * Length) {
            ...
            superExtended = (Vector3.Distance(smoothtarget, transform.position) > superExtendedRange);
        }
        else {
            extended = false;
